define(['echarts', 'threejs'], function (echarts) {
    ...
    app.controller('RobotVRA1200Controller', ['$scope', '$timeout', 'AppComponent', 'dialog', 'AppDataService', '$element', 'websocket',
        function ($scope, $timeout, AppComponent, dialog, AppDataService, $element, websocket) {
            var ctrl = this;

            var appName = 'robot';
            var renderWidth, renderHeight, container, camera, scene, controls, renderer, scale = 95;
            ...
            _.extend(ctrl, {
                ...
                initDeviceSelect: function () {
                    ...
                    AppComponent.deviceselect($('#vra1200_bind_device_box'), {'robot.show': true}, uuid).progress(function (bind) {
                ...
                refreshData: function () {
                    AppDataService.set(appName, 'vr_a1200', AppConfig);
                    dialog.notyWithRefresh('数据配置已完成,2秒后自动刷新页面运行', $scope);
                },
                initData: function () {
                    AppDataService.get(appName, 'vr_a1200').success(function (result) {
                        if (_.isEmpty(result)) {
                            AppConfig = {
                ...
                setDeviceName: function () {
                    ...
                    deviceName = deviceName || '模型-A1200';
                ...
                updateValue: function (data) {
                    ...
                    $('.vr_1200_state_item').each(function () {
                        var ele = $(this);
                        var valueKey = ele.data('key');
                ...
                showState: function (keyword) {
                    ...
                    dialog.show({
                        title: title,
                        'template': 'vr_robot_a1200_state_template',
                        width: 1200,
                        cancel: false,
                        data: {
                            key: keyword,
                ...
                modifyConfig: function () {
                    dialog.show({
                        title: false,
                        template: 'vr_robot_a1200_config_template',
                        width: 1200,
                        data: {
                            entity: AppConfig,
                            direcOptions: [{
                ...
                initRobot: function () {
                    var files = [{
                        ...
                        filename: '1.ctm',
                        offset: {
                            // x: -0.02299468219280243,
                            x: 0,
                            y: -0.24916843324899673,
                            // z: -0.26198869943618774
                            z: -0.29
                        },
                        option: {
                            color: '#209698'
                        ...
                        filename: '2.ctm',
                        offset: {
                            // x: 0.19050825387239456,
                            x: 0.185,
                            // y: -0.6041350364685059,
                            y: -0.32,
                            z: -0.2224762812256813
                        },
                        option: {
                            color: '#209698'
                        }
                        ...
                        filename: '3.ctm',
                        offset: {
                            x: 0.000012315809726715088,
                            // y: -0.9202581942081451,
                            y: -0.914,
                            // z: -0.2164904922246933
                            z: -0.227
                        },
                        option: {
                            color: '#209698'
                        ...
                        filename: '5.ctm',
                        offset: {
                            x: 0.001008572056889534,
                            y: -0.906652182340622,
                            // z: 0.33113478124141693
                            z: 0.315
                        },
                        option: {
                            color: '#209698'
                    ...
                    ctrl.loadCtmFileAsyc(files, parentObject, function (file, object, mesh) {
                        if (file.filename === '1.ctm') {
                            ...
                            object.position.x = 0;
                            object.position.y = 0;
                            object.position.z = -0.17;
                            object.type = 'rotation';
                        } else if (file.filename === '6.ctm') {
                            object.control = 'z';
                            object.position.x = 0;
                            object.position.y = 0;
                            object.position.z = -0.09;
                            object.type = 'rotation';
                        }
                ...
                autoWidth: function () {
                    renderWidth = $('#a1200_webgl_box').width();

                    renderHeight = $('#a1200_webgl_box').height();
                    ...
                    $('.a1200_left').height(winheight - 200);

                    var boxes = $('.vr_a1200_box');
                    var leftWidth = $('.a1200_left').width();
                    boxes.width((leftWidth - 20) / 4);
                    boxes.height(leftHeight - 495);

                    var stateBoxHeight = leftHeight - 525;
                    $('.vr_a1200_state_box').height(stateBoxHeight);

                    var stateItemHeight = (stateBoxHeight - 40) / 4;
                    $('.vr_1200_state_item_wrap').height(stateItemHeight);

                    var stateItemWidth = $('.vr_a1200_state_box').width();
                    $('.vr_1200_state_item_wrap').width((stateItemWidth - 10) / 4);
                },
                ...
                getCtmURL: function (glfile) {
                    return cynovan.r_path + 'robot/vr_a1200/resource/ctm/' + glfile + '?v=' + cynovan.version;
                },
                initWebGL: function () {
                    container = document.getElementById('a1200_webgl_box');
                    ...
                    parentObject.position.x = 12
                    parentObject.position.y = -40;
                    parentObject.rotation.y = 110 * OneDegree;
