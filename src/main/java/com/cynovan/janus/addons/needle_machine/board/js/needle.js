define(['needle_machine/board/js/needle_device_init_data_config'], function (AppDataConfig) {
    var app = angular.module('app');

    app.controller('NeedleDeviceBoardController', ['$scope', '$state', 'DBUtils', 'dialog', 'http', '$element', 'util', 'database', 'AppConfigService', 'BindDevice', '$timeout', 'janus',
        function ($scope, $state, DBUtils, dialog, http, $element, util, database, AppConfigService, BindDevice, $timeout, janus) {

            var ctrl = this;

            $scope.model = database.get('janus-needle-device-model') || 'kanban';
            _.extend(ctrl, {
                initialize: function () {
                    ctrl.initListOption();
                ...
                bindEvent: function () {
                    ...
                    $scope.$watch('model', function (newVal) {
                        database.set('janus-needle-device-model', newVal);
                    });
                ...
                initKanbanOptions: function () {
                    ctrl.kanbanOptions = {
                        ...
                        itemRender: function (data) {
                            var imageUrl = ctrl.getNeedleImage(data);
                ...
                getNeedleImage: function (itemData) {
                    var imageId = _.get(itemData, 'needle.image_id', '');
                    if (imageId) {
                        var imageUrl = util.getImageUrl(imageId, 'needle_machine/img/app-needle.png', true);
                        return `<a class="imgWrap fancy-img" data-type="image" href="${imageUrl + '?thumb=false'}"><img src="${imageUrl}"/></a>`;
                    } else {
                        return `<a class="imgWrap"><img src="resource/needle_machine/img/app-needle.png"/></a>`;
                    }
                },
                showDeviceDetail: function (uuid) {
                    janus.goToMenuDetailByName('机台', uuid);
                },
                bindDeviceToNeedle: function () {
                    BindDevice.bind('needle.show').done(function () {
                        ctrl.refreshTable();
                    });
                },
