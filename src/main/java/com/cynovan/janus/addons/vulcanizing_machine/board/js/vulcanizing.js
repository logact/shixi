define(['vulcanizing_machine/board/js/vulcanizing_device_init_data_config'], function (AppDataConfig) {
    var app = angular.module('app');

    app.controller('VulcanizingDeviceBoardController', ['$scope', '$state', 'DBUtils', 'dialog', 'http', '$element', 'util', 'database', 'AppConfigService', 'BindDevice', '$timeout', 'janus',
        function ($scope, $state, DBUtils, dialog, http, $element, util, database, AppConfigService, BindDevice, $timeout, janus) {

            var ctrl = this;

            $scope.model = database.get('janus-vulcanizing-device-model') || 'kanban';
            _.extend(ctrl, {
                initialize: function () {
                    ctrl.initListOption();
                ...
                bindEvent: function () {
                    ...
                    $scope.$watch('model', function (newVal) {
                        database.set('janus-vulcanizing-device-model', newVal);
                    });
                ...
                initKanbanOptions: function () {
                    ctrl.kanbanOptions = {
                        ...
                        itemRender: function (data) {
                            var imageUrl = ctrl.getVulcanizingImage(data);
                            var cls = data.online.toString() === 'true' ? 'active' : '';
                            var totalTime = _.get(data, 'dynamicData.16_value', '0');
                ...
                getVulcanizingImage: function (itemData) {
                    var imageId = _.get(itemData, 'vulcanizing.image_id', '');
                    if (imageId) {
                        var imageUrl = util.getImageUrl(imageId, 'vulcanizing_machine/img/app-vulcanizing.png', true);
                        return `<a class="imgWrap fancy-img" data-type="image" href="${imageUrl + '?thumb=false'}"><img src="${imageUrl}"/></a>`;
                    } else {
                        return `<a class="imgWrap"><img src="resource/vulcanizing_machine/img/app-vulcanizing.png"/></a>`;
                    }
                },
                showDeviceDetail: function (uuid) {
                    janus.goToMenuDetailByName("机台", uuid)
                },
                bindDeviceToVulcanizing: function () {
                    BindDevice.bind('vulcanizing.show').done(function () {
                        ctrl.refreshTable();
                    });
                },
