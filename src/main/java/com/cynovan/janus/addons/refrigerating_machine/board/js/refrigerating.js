define(['refrigerating_machine/board/js/refrigerating_device_init_data_config'], function (AppDataConfig) {
    var app = angular.module('app');

    app.controller('RefrigeratingDeviceBoardController', ['$scope', '$state', 'DBUtils', 'dialog', 'http', '$element', 'util', 'database', 'AppConfigService', 'BindDevice', '$timeout', 'janus',
        function ($scope, $state, DBUtils, dialog, http, $element, util, database, AppConfigService, BindDevice, $timeout, janus) {

            var ctrl = this;

            $scope.model = database.get('janus-refrigerating-device-model') || 'kanban';
            _.extend(ctrl, {
                initialize: function () {
                    ctrl.initListOption();
                ...
                bindEvent: function () {
                    ...
                    $scope.$watch('model', function (newVal) {
                        database.set('janus-refrigerating-device-model', newVal);
                    });
                ...
                initKanbanOptions: function () {
                    ctrl.kanbanOptions = {
                        ...
                        itemRender: function (data) {
                            var imageUrl = ctrl.getRefrigeratingImage(data);
                            var cls = data.online.toString() === 'true' ? 'active' : '';
                            var totalTime = _.get(data, 'dynamicData.01_value', '0');
                ...
                getRefrigeratingImage: function (itemData) {
                    var imageId = _.get(itemData, 'refrigerating.image_id', '');
                    if (imageId) {
                        var imageUrl = util.getImageUrl(imageId, 'refrigerating_machine/img/app-refrigerating.png', true);
                        return `<a class="imgWrap fancy-img" data-type="image" href="${imageUrl + '?thumb=false'}"><img src="${imageUrl}"/></a>`;
                    } else {
                        return `<a class="imgWrap"><img src="resource/refrigerating_machine/img/app-refrigerating.png"/></a>`;
                    }
                },
                showDeviceDetail: function (uuid) {
                    janus.goToMenuDetailByName('机台', uuid);
                },
                bindDeviceToRefrigerating: function () {
                    BindDevice.bind('refrigerating.show').done(function () {
                        ctrl.refreshTable();
                    });
                },
