define(['cutting_machine/board/js/cutting_device_init_data_config'], function (AppDataConfig) {
    var app = angular.module('app');

    app.controller('CuttingDeviceBoardController', ['$scope', '$state', 'DBUtils', 'dialog', 'http', '$element', 'util', 'database', 'AppConfigService', 'BindDevice', '$timeout', 'janus',
        function ($scope, $state, DBUtils, dialog, http, $element, util, database, AppConfigService, BindDevice, $timeout, janus) {

            var ctrl = this;

            $scope.model = database.get('janus-cutting-device-model') || 'kanban';
            _.extend(ctrl, {
                initialize: function () {
                    ctrl.initListOption();
                ...
                bindEvent: function () {
                    ...
                    $scope.$watch('model', function (newVal) {
                        database.set('janus-cutting-device-model', newVal);
                    });
                ...
                initKanbanOptions: function () {
                    ctrl.kanbanOptions = {
                        ...
                        itemRender: function (data) {
                            var imageUrl = ctrl.getCuttingImage(data);
                            var cls = data.online.toString() === 'true' ? 'active' : '';
                            var totalTime = _.get(data, 'dynamicData.09_value', '0');
                ...
                getCuttingImage: function (itemData) {
                    var imageId = _.get(itemData, 'cutting.image_id', '');
                    if (imageId) {
                        var imageUrl = util.getImageUrl(imageId, 'cutting_machine/img/app-cutting.png', true);
                        return `<a class="imgWrap fancy-img" data-type="image" href="${imageUrl + '?thumb=false'}"><img src="${imageUrl}"/></a>`;
                    } else {
                        return `<a class="imgWrap"><img src="resource/cutting_machine/img/app-cutting.png"/></a>`;
                    }
                },
                showDeviceDetail: function (uuid) {
                    janus.goToMenuDetailByName('机台', uuid);
                },
                bindDeviceToCutting: function () {
                    BindDevice.bind('cutting.show').done(function () {
                        ctrl.refreshTable();
                    });
                },
