define(['sewing_machine/board/js/sewing_device_init_data_config'], function (AppDataConfig) {
    var app = angular.module('app');

    app.controller('SewingDeviceBoardController', ['$scope', '$state', 'DBUtils', 'dialog', 'http', '$element', 'util', 'database', 'AppConfigService', 'BindDevice', '$timeout', 'janus',
        function ($scope, $state, DBUtils, dialog, http, $element, util, database, AppConfigService, BindDevice, $timeout, janus) {

            var ctrl = this;

            $scope.model = database.get('janus-sewing-device-model') || 'kanban';
            _.extend(ctrl, {
                initialize: function () {
                    ctrl.initListOption();
                ...
                bindEvent: function () {
                    ...
                    $scope.$watch('model', function (newVal) {
                        database.set('janus-sewing-device-model', newVal);
                    });
                ...
                initKanbanOptions: function () {
                    ctrl.kanbanOptions = {
                        ...
                        itemRender: function (data) {
                            var imageUrl = ctrl.getSewingImage(data);
                ...
                getSewingImage: function (itemData) {
                    var imageId = _.get(itemData, 'sewing.image_id', '');
                    if (imageId) {
                        var imageUrl = util.getImageUrl(imageId, 'sewing_machine/img/app-sewing.png', true);
                        return `<a class="imgWrap fancy-img" data-type="image" href="${imageUrl + '?thumb=false'}"><img src="${imageUrl}"/></a>`;
                    } else {
                        return `<a class="imgWrap"><img src="resource/sewing_machine/img/app-sewing.png"/></a>`;
                    }
                },
                showDeviceDetail: function (uuid) {
                    janus.goToMenuDetailByName('机台', uuid);
                },
                bindDeviceToSewing: function () {
                    BindDevice.bind('sewing.show').done(function () {
                        ctrl.refreshTable();
                    });
                },
